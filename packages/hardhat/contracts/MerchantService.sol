...
contract MerchantService is Ownable, ReentrancyGuard {
     ...
     */
    function receivePayment(address _merchant, uint256 _amount) 
        external 
        nonReentrant 
    {
        ...
        require(
            liquidToken.balanceOf(msg.sender) >= _amount,
            "Insufficient balance"
        );
        
        // Check allowance
        require(
            liquidToken.allowance(msg.sender, address(this)) >= _amount,
        ...
        bool paymentSuccess = liquidToken.transferFrom(
            msg.sender,
            _merchant,
            _amount
        );
     ...
     * @param _merchant The merchant's address
     */
    function getMerchantTransactions(address _merchant) 
        external 
        view 
     ...
     */
    function getStudentTransactions(address _student) 
        external 
        view 
        returns (Transaction[] memory) 
    {
